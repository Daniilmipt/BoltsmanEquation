...
#ifndef TASK_RELAX_KOROBOVGRID_H
#define TASK_RELAX_KOROBOVGRID_H
...
public:
    double func_H (int p, int s, int z) {  // p is prime
        double sum = 0;
    ...
    coef_t korobov_coef(int prime, int n) {
        if (n == 8) {  // TODO: make mith switch
            ...
            if (prime == 400009){
                coef_t a{1, 3491, 186811, 142531, 364534, 159565, 228887, 226544};
                return a;
            }
            if (prime == 500009) {
    ...
    KorobovGrid () = default;
    KorobovGrid (const KorobovGrid& grid) : grid_t(grid) {}
    KorobovGrid (KorobovGrid&& grid) : grid_t(std::move(grid)) {}

    KorobovGrid (const int prime, const coef_t& coef, bool haveShuffle, bool haveShift)
    ...
    KorobovGrid (const int prime, const int n, bool haveShaffle, bool haveShift)
...
#endif //TASK_RELAX_KOROBOVGRID_H
